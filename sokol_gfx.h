...
/*
   ...
   surfaces to a specific color), and .swapchain is an sg_swapchain struct all
   the required information to render into the swapchain's surfaces.

        To start an 'offscreen pass' into sokol-gfx image objects, an
   ...
   attachments });
    ...
    --- optionally update shader uniform data with:
            ...
            sg_apply_uniforms(sg_shader_stage stage, int ub_index, const
   sg_range* data)
   ...
   the viewport and scissor rectangles to cover the entire framebuffer.
    ...
    --- to update (overwrite) the content of buffer and image resources, call:
        ...
        countermeasure to avoid the CPU scribbling over data the GPU is
   currently using, or the CPU having to wait for the GPU
   ...
   (sg_update_buffer, sg_append_buffer and sg_update_image) per frame Not all of
   those limit values are used by all backends, but it is good practice to
   provide them none-the-less.
   ...
   sg_make_shader(myshader_shader_desc(sg_query_backend()));
    ...
    - Shader code or shader binary blobs for the vertex- and fragment-
   shader-stage:
        - for the desktop GL backend, source code can be provided in '#version
   410' or
          '#version 430', version 430 is required for storage buffer support,
   but note that this is not available on macOS
   ...
   blobs, the source code should be in HLSL4.0 (for best compatibility) or
   alternatively in HLSL5.0 syntax (other versions may work but are not tested),
   NOTE: when shader source code is provided for the D3D11 backend, sokol-gfx
   will dynamically load 'd3dcompiler_47.dll'
   ...
   "ps_4_0")
    ...
    - Depending on backend, information about the input vertex attributes used
   by the vertex shader:
        - Metal: no information needed since vertex attributes are always bound
          by their attribute location defined in the shader via
   '[[attribute(N)]]'
        - WebGPU: no information needed since vertex attributes are always
          bound by their attribute location defined in the shader via
   `@location(N)`
   ...
   location can be defined with 'layout(location = N)', PLEASE NOTE that the
   name-lookup method may be removed at some point
   ...
   structured data fromt the CPU side to the shaders. They are similar to data
   ...
   to GL 4.1)
    - all GLES3 platforms (WebGL2, iOS, Android - with the option that support
   on Android may be added at a later point)

    Currently only 'readonly' storage buffers are supported (meaning it's not
   possible to write to storage buffers from shaders).

    To use storage buffers, the following steps are required:

        - write a shader which uses storage buffers (also see the example links
   below)
   ...
   will be taken care of automatically)
            - which storage buffer bind slots on the vertex- and fragment-stage
              are occupied
            - whether the storage buffer on that bind slot is readonly (this is
   ...
   { ... }`
        - do NOT annotate storage buffers with `layout(...)`, sokol-shdc will
   take care of that
   ...
   sokol-shdc):
        ...
        D3D11:
            ...
            - in D3D11, storage buffers and textures share the same bind slots,
   sokol-gfx reserves shader resource slots 0..15 for textures and 16..23 for
   storage buffers.
            - e.g. in HLSL, storage buffer bindings start at register(t16) no
   matter the shader stage
   ...
   and storage-buffers, all are bound to the same 'buffer bind slots' with the
            ...
            - this means in MSL, storage buffer bindings start at [[buffer(12)]]
   in the vertex shaders, and at [[buffer(4)]] in fragment shaders
   ...
   shader
        ...
        WebGPU:
            - in WGSL, use the following bind locations for the various shader
   resource types:
            - vertex shader stage:
    ...
    - D3D11 can only convert *normalized* vertex formats to
      ...
      vertex shader inputs, those can only be uses with the *ivecn* vertex
   shader input types when D3D11 is used as backend (GL and Metal can use both
   formats)
    ...
    For a vertex input layout which works on all platforms, only use the
   following vertex formats, and if needed "expand" the normalized vertex shader
   ...
   WebGL2 code.
    ...
    - when writing WGSL shader code by hand, a specific bind-slot convention
      ...
      4 uniform blocks per shader stage.
        - Vertex shader uniform block bindings must start at `@group(0)
   @binding(0)`
        - Fragment shader uniform blocks bindings must start at `@group(0)
   @binding(4)`
      ...
      Note that the actual number of allowed per-stage texture- and
   sampler-bindings in sokol-gfx is currently lower than the above ranges
   (currently only up to 12 textures, 8 samplers and 8 storage buffers are
   allowed per shader stage).

      If you use sokol-shdc to generate WGSL shader code, you don't need to
   worry about the above binding convention since sokol-shdc assigns bind slots
      automatically.
   ...
   be fixed in a future WebGPU update.
   ...
   WebGPU implementation seems to recognize this.
    ...
    - The vertex format SG_VERTEXFORMAT_UINT10_N2 is currently not supported
   because WebGPU lacks a matching vertex format (this is currently being worked
   on though, as soon as the vertex format shows up in webgpu.h, sokol_gfx.h
   will add support.

    - Likewise, the following sokol-gfx vertex formats are not supported in
   WebGPU: R16, R16SN, RG16, RG16SN, RGBA16, RGBA16SN and all PVRTC compressed
   format. Unlike unsupported vertex formats, unsupported pixel formats can be
   queried in cross-backend code via sg_query_pixel_format() though.
...
/*
    ...
    sg_sampler      sampler object describing how a texture is sampled in a
...
// Shader stages increased to 3 to support compute shaders
enum {
  SG_INVALID_ID = 0,
...
typedef struct sg_bindings {
  ...
  sg_stage_bindings cs; // compute shader binding, new field
  uint32_t _end_canary;
} sg_bindings;
...
/*
   ...
   GL backends):
        ...
        - reflection info for textures used in the shader stage:
            ...
            - the image-sample type (SG_IMAGESAMPLETYPE_xxx, default is
   SG_IMAGESAMPLETYPE_FLOAT)
...
typedef struct sg_shader_stage_desc {
  ...
  sg_shader_storage_buffer_desc
      storage_buffers[SG_MAX_SHADERSTAGE_STORAGEBUFFERS];
  sg_shader_image_desc images[SG_MAX_SHADERSTAGE_IMAGES];
  sg_shader_sampler_desc samplers[SG_MAX_SHADERSTAGE_SAMPLERS];
  sg_shader_image_sampler_pair_desc
      image_sampler_pairs[SG_MAX_SHADERSTAGE_IMAGESAMPLERPAIRS];
...
typedef struct sg_shader_desc {
  ...
  sg_shader_stage_desc cs; // compute shader stage, new field
  bool is_compute;
  const char *label;
  uint32_t _end_canary;
} sg_shader_desc;
...
typedef struct sg_trace_hooks {
  ...
  void (*make_image)(const sg_image_desc *desc, sg_image result,
                     void *user_data);
  void (*make_sampler)(const sg_sampler_desc *desc, sg_sampler result,
                       void *user_data);
  ...
  void (*update_image)(sg_image img, const sg_image_data *data,
                       void *user_data);
  void (*append_buffer)(sg_buffer buf, const sg_range *data, int result,
                        void *user_data);
  ...
  void (*apply_uniforms)(sg_shader_stage stage, int ub_index,
  ...
  void (*init_image)(sg_image img_id, const sg_image_desc *desc,
                     void *user_data);
  void (*init_sampler)(sg_sampler smp_id, const sg_sampler_desc *desc,
                       void *user_data);
  ...
  void (*init_attachments)(sg_attachments atts_id,
                           const sg_attachments_desc *desc, void *user_data);
...
#define _SG_LOG_ITEMS                                                          \
  ...
  _SG_LOGITEM_XMACRO(GL_VERTEX_ATTRIBUTE_NOT_FOUND_IN_SHADER,                  \
                     "vertex attribute not found in shader (gl)")              \
  _SG_LOGITEM_XMACRO(GL_TEXTURE_NAME_NOT_FOUND_IN_SHADER,                      \
                     "texture name not found in shader (gl)")                  \
  ...
  _SG_LOGITEM_XMACRO(METAL_VERTEX_SHADER_ENTRY_NOT_FOUND,                      \
                     "vertex shader entry function not found (metal)")         \
  _SG_LOGITEM_XMACRO(METAL_FRAGMENT_SHADER_ENTRY_NOT_FOUND,                    \
                     "fragment shader entry not found (metal)")                \
  ...
  _SG_LOGITEM_XMACRO(                                                          \
      ...
      "shader uses too many sampled images on shader stage (wgpu)")            \
  _SG_LOGITEM_XMACRO(WGPU_SHADER_TOO_MANY_SAMPLERS,                            \
                     "shader uses too many samplers on shader stage (wgpu)")   \
  _SG_LOGITEM_XMACRO(                                                          \
  ...
  _SG_LOGITEM_XMACRO(                                                          \
      ...
      "sg_install_trace_hooks() called, but SG_TRACE_HOOKS is not defined")    \
  ...
  _SG_LOGITEM_XMACRO(VALIDATE_BUFFERDESC_SIZE,                                 \
                     "sg_buffer_desc.size and .data.size cannot both be 0")    \
  _SG_LOGITEM_XMACRO(VALIDATE_BUFFERDESC_DATA,                                 \
                     "immutable buffers must be initialized with data "        \
                     "(sg_buffer_desc.data.ptr and sg_buffer_desc.data.size)") \
  _SG_LOGITEM_XMACRO(VALIDATE_BUFFERDESC_DATA_SIZE,                            \
                     "immutable buffer data size differs from buffer size")    \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_BUFFERDESC_NO_DATA,                                             \
      "dynamic/stream usage buffers cannot be initialized with data")          \
  ...
  _SG_LOGITEM_XMACRO(VALIDATE_SAMPLERDESC_MINFILTER_NONE,                      \
                     "sg_sampler_desc.min_filter cannot be SG_FILTER_NONE")    \
  _SG_LOGITEM_XMACRO(VALIDATE_SAMPLERDESC_MAGFILTER_NONE,                      \
                     "sg_sampler_desc.mag_filter cannot be SG_FILTER_NONE")    \
  _SG_LOGITEM_XMACRO(                                                          \
  ...
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_SOURCE,                               \
                     "shader source code required")                            \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_BYTECODE,                             \
                     "shader byte code required")                              \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_SOURCE_OR_BYTECODE,                   \
                     "shader source or byte code required")                    \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_NO_BYTECODE_SIZE,                     \
                     "shader byte code length (in bytes) required")            \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_NO_CONT_UBS,                          \
                     "shader uniform blocks must occupy continuous slots")     \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_NO_CONT_UB_MEMBERS,                   \
                     "uniform block members must occupy continuous slots")     \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_NO_UB_MEMBERS,                        \
                     "GL backend requires uniform block member declarations")  \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_UB_MEMBER_NAME,                       \
                     "uniform block member name missing")                      \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SHADERDESC_UB_SIZE_MISMATCH,                                    \
      "size of uniform block members doesn't match uniform block size")        \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_UB_ARRAY_COUNT,                       \
  ...
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_NO_CONT_STORAGEBUFFERS,               \
                     "shader stage storage buffers must occupy continuous "    \
                     "slots (sg_shader_desc.vs|fs.storage_buffers[])")         \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_STORAGEBUFFER_READONLY,               \
                     "shader stage storage buffers must be readonly "          \
                     "(sg_shader_desc.vs|fs.storage_buffers[].readonly)")      \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_NO_CONT_IMAGES,                       \
                     "shader stage images must occupy continuous slots "       \
                     "(sg_shader_desc.vs|fs.images[])")                        \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_NO_CONT_SAMPLERS,                     \
                     "shader stage samplers must occupy continuous slots "     \
                     "(sg_shader_desc.vs|fs.samplers[])")                      \
  _SG_LOGITEM_XMACRO(                                                          \
  ...
  _SG_LOGITEM_XMACRO(                                                          \
      ...
      "shader stage: image-sampler-pairs must be named in GL "                 \
      "(sg_shader_desc.vs|fs.image_sampler_pairs[].name)")                     \
  _SG_LOGITEM_XMACRO(                                                          \
  ...
  _SG_LOGITEM_XMACRO(                                                          \
      ...
      "shader stage: image-sampler-pair has .image_slot != 0 but .used field " \
      "not true")                                                              \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SHADERDESC_IMAGE_SAMPLER_PAIR_HAS_SAMPLER_BUT_NOT_USED,         \
      "shader stage: image-sampler-pair .sampler_slot != 0 but .used field "   \
      "not true")                                                              \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_NONFILTERING_SAMPLER_REQUIRED,        \
                     "shader stage: image sample type UNFILTERABLE_FLOAT, "    \
                     "UINT, SINT can only be used with NONFILTERING sampler")  \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_COMPARISON_SAMPLER_REQUIRED,          \
                     "shader stage: image sample type DEPTH can only be used " \
                     "with COMPARISON sampler")                                \
  _SG_LOGITEM_XMACRO(                                                          \
  ...
  _SG_LOGITEM_XMACRO(                                                          \
      ...
      "shader stage: one or more samplers are not referenced by "              \
      "image-sampler-pairs "                                                   \
      "(sg_shader_desc.vs|fs.image_sampler_pairs[].sampler_slot)")             \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SHADERDESC_NO_CONT_IMAGE_SAMPLER_PAIRS,                         \
      "shader stage image-sampler-pairs must occupy continuous slots "         \
      "(sg_shader_desc.vs|fs.image_samplers[])")                               \
  _SG_LOGITEM_XMACRO(                                                          \
  ...
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_PIPELINE_VALID,                             \
                     ...
                     "not in valid state")                                     \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_VBS,                                        \
                     "sg_apply_bindings: number of vertex buffers doesn't "    \
                     "match number of pipeline vertex layouts")                \
  ...
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_VS_EXPECTED_IMAGE_BINDING,                  \
                     "sg_apply_bindings: image binding on vertex stage is "    \
                     "missing or the image handle is invalid")                 \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ABND_VS_IMG_EXISTS,                                             \
      "sg_apply_bindings: image bound to vertex stage no longer alive")        \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_VS_IMAGE_TYPE_MISMATCH,                     \
                     "sg_apply_bindings: type of image bound to vertex stage " \
                     "doesn't match shader desc")                              \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_VS_IMAGE_MSAA,                              \
                     "sg_apply_bindings: cannot bind image with "              \
                     "sample_count>1 to vertex stage")                         \
  _SG_LOGITEM_XMACRO(                                                          \
  ...
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_VS_EXPECTED_SAMPLER_BINDING,                \
                     "sg_apply_bindings: sampler binding on vertex stage is "  \
                     "missing or the sampler handle is invalid")               \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ABND_VS_UNEXPECTED_SAMPLER_COMPARE_NEVER,                       \
      "sg_apply_bindings: shader expects SG_SAMPLERTYPE_COMPARISON on vertex " \
      "stage but sampler has SG_COMPAREFUNC_NEVER")                            \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ABND_VS_EXPECTED_SAMPLER_COMPARE_NEVER,                         \
      "sg_apply_bindings: shader expects SG_SAMPLERTYPE_FILTERING or "         \
      "SG_SAMPLERTYPE_NONFILTERING on vertex stage but sampler doesn't have "  \
      "SG_COMPAREFUNC_NEVER")                                                  \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ABND_VS_EXPECTED_NONFILTERING_SAMPLER,                          \
      "sg_apply_bindings: shader expected SG_SAMPLERTYPE_NONFILTERING on "     \
      "vertex stage, but sampler has SG_FILTER_LINEAR filters")                \
  _SG_LOGITEM_XMACRO(                                                          \
  ...
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_VS_EXPECTED_STORAGEBUFFER_BINDING,          \
                     "sg_apply_bindings: storage buffer binding on vertex "    \
                     "stage is missing or the buffer handle is invalid")       \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_VS_STORAGEBUFFER_EXISTS,                    \
                     "sg_apply_bindings: storage buffer bound to vertex "      \
                     "stage no longer alive")                                  \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_VS_STORAGEBUFFER_BINDING_BUFFERTYPE,        \
                     "sg_apply_bindings: buffer bound to vertex stage "        \
                     "storage buffer slot is not of type storage buffer")      \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ABND_VS_UNEXPECTED_STORAGEBUFFER_BINDING,                       \
      "sg_apply_bindings: unexpected storage buffer binding on vertex stage")  \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_FS_EXPECTED_IMAGE_BINDING,                  \
                     "sg_apply_bindings: image binding on fragment stage is "  \
                     "missing or the image handle is invalid")                 \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ABND_FS_IMG_EXISTS,                                             \
      "sg_apply_bindings: image bound to fragment stage no longer alive")      \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_FS_IMAGE_TYPE_MISMATCH,                     \
                     "sg_apply_bindings: type of image bound to fragment "     \
                     "stage doesn't match shader desc")                        \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_FS_IMAGE_MSAA,                              \
                     "sg_apply_bindings: cannot bind image with "              \
                     "sample_count>1 to fragment stage")                       \
  _SG_LOGITEM_XMACRO(                                                          \
  ...
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_FS_EXPECTED_SAMPLER_BINDING,                \
                     "sg_apply_bindings: sampler binding on fragment stage "   \
                     "is missing or the sampler handle is invalid")            \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ABND_FS_UNEXPECTED_SAMPLER_COMPARE_NEVER,                       \
      "sg_apply_bindings: shader expects SG_SAMPLERTYPE_COMPARISON on "        \
      "fragment stage but sampler has SG_COMPAREFUNC_NEVER")                   \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ABND_FS_EXPECTED_SAMPLER_COMPARE_NEVER,                         \
      "sg_apply_bindings: shader expects SG_SAMPLERTYPE_FILTERING on "         \
      "fragment stage but sampler doesn't have SG_COMPAREFUNC_NEVER")          \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ABND_FS_EXPECTED_NONFILTERING_SAMPLER,                          \
      "sg_apply_bindings: shader expected SG_SAMPLERTYPE_NONFILTERING on "     \
      "fragment stage, but sampler has SG_FILTER_LINEAR filters")              \
  _SG_LOGITEM_XMACRO(                                                          \
  ...
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_FS_EXPECTED_STORAGEBUFFER_BINDING,          \
                     "sg_apply_bindings: storage buffer binding on fragment "  \
                     "stage is missing or the buffer handle is invalid")       \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_FS_STORAGEBUFFER_EXISTS,                    \
                     "sg_apply_bindings: storage buffer bound to fragment "    \
                     "stage no longer alive")                                  \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_FS_STORAGEBUFFER_BINDING_BUFFERTYPE,        \
                     "sg_apply_bindings: buffer bound to frahment stage "      \
                     "storage buffer slot is not of type storage buffer")      \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_FS_UNEXPECTED_STORAGEBUFFER_BINDING,        \
                     "sg_apply_bindings: unexpected storage buffer binding "   \
                     "on fragment stage")                                      \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_AUB_NO_PIPELINE,                                                \
      "sg_apply_uniforms: must be called after sg_apply_pipeline()")           \
  _SG_LOGITEM_XMACRO(VALIDATE_AUB_NO_UB_AT_SLOT,                               \
                     "sg_apply_uniforms: no uniform block declaration at "     \
                     "this shader stage UB slot")                              \
  _SG_LOGITEM_XMACRO(VALIDATE_AUB_SIZE,                                        \
                     "sg_apply_uniforms: data size doesn't match declared "    \
                     "uniform block size")                                     \
...
SOKOL_GFX_API_DECL void sg_apply_viewport(int x, int y, int width, int height,
                                          bool origin_top_left);
SOKOL_GFX_API_DECL void sg_apply_viewportf(float x, float y, float width,
                                           float height, bool origin_top_left);
SOKOL_GFX_API_DECL void sg_apply_scissor_rect(int x, int y, int width,
                                              int height, bool origin_top_left);
SOKOL_GFX_API_DECL void sg_apply_scissor_rectf(float x, float y, float width,
                                               float height,
                                               bool origin_top_left);
...
SOKOL_GFX_API_DECL void sg_apply_uniforms(sg_shader_stage stage, int ub_index,
                                          const sg_range *data);
SOKOL_GFX_API_DECL void sg_draw(int base_element, int num_elements,
                                int num_instances);
...
SOKOL_GFX_API_DECL int sg_query_row_pitch(sg_pixel_format fmt, int width,
                                          int row_align_bytes);
SOKOL_GFX_API_DECL int sg_query_surface_pitch(sg_pixel_format fmt, int width,
                                              int height, int row_align_bytes);
...
typedef struct sg_d3d11_shader_info {
  const void *vs_cbufs[SG_MAX_SHADERSTAGE_UBS]; // ID3D11Buffer* (vertex stage
                                                // constant buffers)
  const void *fs_cbufs[SG_MAX_SHADERSTAGE_UBS]; // ID3D11Buffer* (fragment stage
                                                // constant buffers)
...
typedef struct sg_wgpu_shader_info {
  ...
  const void *cs_mod; // WGPUShaderModule
...
// Metal: return __bridge-casted MTLRenderCommandEncoder in current pass (or
// zero if outside pass)
...
inline void sg_apply_uniforms(sg_shader_stage stage, int ub_index,
                              const sg_range &data) {
...
_SOKOL_PRIVATE uint32_t _sg_slot_alloc(_sg_pool_t *pool, _sg_slot_t *slot,
                                       int slot_index);
...
typedef struct {
  ...
  _sg_shader_image_sampler_t
      image_samplers[SG_MAX_SHADERSTAGE_IMAGESAMPLERPAIRS];
} _sg_shader_stage_t;
...
_SOKOL_PRIVATE void _sg_shader_common_init(_sg_shader_common_t *cmn,
  ...
  for (int stage_index = 0; stage_index < 3; stage_index++) {
    ...
    const sg_shader_stage_desc *stage_desc = 0;
    if (stage_index == SG_SHADERSTAGE_VS) {
      stage_desc = &desc->vs;
    } else if (stage_index == SG_SHADERSTAGE_FS) {
      stage_desc = &desc->fs;
    } else if (stage_index == SG_SHADERSTAGE_CS) {
      stage_desc = &desc->cs;
    }
    ...
    for (int img_index = 0; img_index < SG_MAX_SHADERSTAGE_IMAGES;
         img_index++) {
    ...
    for (int smp_index = 0; smp_index < SG_MAX_SHADERSTAGE_SAMPLERS;
         smp_index++) {
    ...
    for (int img_smp_index = 0;
         img_smp_index < SG_MAX_SHADERSTAGE_IMAGESAMPLERPAIRS;
         img_smp_index++) {
      const sg_shader_image_sampler_pair_desc *img_smp_desc =
          &stage_desc->image_sampler_pairs[img_smp_index];
      if (!img_smp_desc->used) {
        break;
      }
      SOKOL_ASSERT((img_smp_desc->image_slot >= 0) &&
                   (img_smp_desc->image_slot < stage->num_images));
      stage->image_samplers[img_smp_index].image_slot =
          img_smp_desc->image_slot;
      SOKOL_ASSERT((img_smp_desc->sampler_slot >= 0) &&
                   (img_smp_desc->sampler_slot < stage->num_samplers));
      stage->image_samplers[img_smp_index].sampler_slot =
          img_smp_desc->sampler_slot;
      stage->num_image_samplers++;
    }
    SOKOL_ASSERT(stage->num_storage_buffers == 0);
    for (int sbuf_index = 0; sbuf_index < SG_MAX_SHADERSTAGE_STORAGEBUFFERS;
         sbuf_index++) {
      const sg_shader_storage_buffer_desc *sbuf_desc =
          &stage_desc->storage_buffers[sbuf_index];
      if (!sbuf_desc->used) {
        break;
      }
...
#define _SG_GL_TEXTURE_SAMPLER_CACHE_SIZE                                      \
  (SG_MAX_SHADERSTAGE_IMAGESAMPLERPAIRS * SG_NUM_SHADER_STAGES)
#define _SG_GL_STORAGEBUFFER_STAGE_INDEX_PITCH                                 \
  (SG_MAX_SHADERSTAGE_STORAGEBUFFERS)
...
typedef struct {
  ...
  GLuint stage_storage_buffers[SG_NUM_SHADER_STAGES]
                              [SG_MAX_SHADERSTAGE_STORAGEBUFFERS];
  ...
  GLuint prog;
  _sg_gl_cache_texture_sampler_bind_slot
      texture_samplers[_SG_GL_TEXTURE_SAMPLER_CACHE_SIZE];
...
#define _SG_WGPU_NUM_BINDGROUPS                                                \
  (2) // 0: uniforms, 1: images and sampler on both shader stages
...
#define _SG_WGPU_MAX_BINDGROUP_ENTRIES                                         \
  (SG_NUM_SHADER_STAGES *                                                      \
   (SG_MAX_SHADERSTAGE_IMAGES + SG_MAX_SHADERSTAGE_SAMPLERS +                  \
    SG_MAX_SHADERSTAGE_STORAGEBUFFERS))
...
typedef struct {
  ...
  _sg_shader_common_t cmn;
  struct {
    _sg_wgpu_shader_stage_t stage[3];
...
typedef struct {
  ...
  WGPUComputePassEncoder comp_enc;
...
typedef struct {
  ...
  int num_fs_smps;
  int num_fs_sbufs;
  int num_cs_sbufs; // New line for compute shader storage buffers
  ...
  _sg_buffer_t
      *cs_sbufs[SG_MAX_SHADERSTAGE_STORAGEBUFFERS]; // New line for compute
...
static void _sg_log(sg_log_item log_item, uint32_t log_level, const char *msg,
                    uint32_t line_nr) {
...
#endif
    _sg.desc.logger.func("sg", log_level, log_item, msg, line_nr, filename,
...
_SOKOL_PRIVATE _sg_recti_t _sg_clipi(int x, int y, int w, int h, int clip_width,
                                     int clip_height) {
...
_SOKOL_PRIVATE sg_resource_state _sg_dummy_create_pipeline(
    _sg_pipeline_t *pip, _sg_shader_t *shd, const sg_pipeline_desc *desc) {
...
_SOKOL_PRIVATE sg_resource_state _sg_dummy_create_attachments(
    _sg_attachments_t *atts, _sg_image_t **color_images,
    _sg_image_t **resolve_images, _sg_image_t *ds_img,
    const sg_attachments_desc *desc) {
...
_SOKOL_PRIVATE void _sg_dummy_apply_viewport(int x, int y, int w, int h,
                                             bool origin_top_left) {
  _SOKOL_UNUSED(x);
  _SOKOL_UNUSED(y);
  _SOKOL_UNUSED(w);
...
_SOKOL_PRIVATE void _sg_dummy_apply_scissor_rect(int x, int y, int w, int h,
                                                 bool origin_top_left) {
  _SOKOL_UNUSED(x);
  _SOKOL_UNUSED(y);
  _SOKOL_UNUSED(w);
...
_SOKOL_PRIVATE void _sg_dummy_apply_uniforms(sg_shader_stage stage_index,
                                             int ub_index,
                                             const sg_range *data) {
...
_SOKOL_PRIVATE void _sg_dummy_draw(int base_element, int num_elements,
                                   int num_instances) {
...
#define _SG_GL_FUNCS                                                           \
  ...
  _SG_XMACRO(glFramebufferRenderbuffer, void,                                  \
             (GLenum target, GLenum attachment, GLenum renderbuffertarget,     \
              GLuint renderbuffer))                                            \
  _SG_XMACRO(glCompressedTexImage2D, void,                                     \
  ...
  _SG_XMACRO(glBlitFramebuffer, void,                                          \
...
_SOKOL_PRIVATE void
_sg_gl_init_pixelformats_float(bool has_colorbuffer_float,
...
_SOKOL_PRIVATE void _sg_gl_cache_bind_storage_buffer(int stage, int slot,
                                                     GLuint buffer) {
  SOKOL_ASSERT((stage >= 0) && (stage < SG_NUM_SHADER_STAGES));
  SOKOL_ASSERT((slot >= 0) && (slot < SG_MAX_SHADERSTAGE_STORAGEBUFFERS));
...
_SOKOL_PRIVATE void _sg_gl_cache_clear_texture_sampler_bindings(bool force) {
  _SG_GL_CHECK_ERROR();
  for (int i = 0; (i < _SG_GL_TEXTURE_SAMPLER_CACHE_SIZE) &&
...
_SOKOL_PRIVATE void _sg_gl_cache_bind_texture_sampler(int slot_index,
                                                      GLenum target,
                                                      GLuint texture,
                                                      GLuint sampler) {
...
_SOKOL_PRIVATE void
_sg_gl_cache_restore_texture_sampler_binding(int slot_index) {
  SOKOL_ASSERT((slot_index >= 0) &&
               (slot_index < _SG_GL_TEXTURE_SAMPLER_CACHE_SIZE));
  _sg_gl_cache_texture_sampler_bind_slot *slot =
      &_sg.gl.cache.stored_texture_sampler;
  if (slot->texture != 0) {
    ...
    _sg_gl_cache_bind_texture_sampler(slot_index, slot->target, slot->texture,
                                      slot->sampler);
    slot->target = 0;
    slot->texture = 0;
    slot->sampler = 0;
...
_sg_gl_create_image(_sg_image_t *img, const sg_image_desc *desc) {
  ...
  // if this is a MSAA render target, a render buffer object will be created
  // instead of a regulat texture (since GLES3 has no multisampled texture
  // objects)
          ...
          if ((SG_IMAGETYPE_2D == img->cmn.type) ||
            ...
            if (is_compressed) {
              ...
              glCompressedTexImage2D(gl_img_target, mip_index,
                                     gl_internal_format, mip_width, mip_height,
                                     0, data_size, data_ptr);
            } else {
            ...
            if (is_compressed) {
              const GLsizei data_size =
                  (GLsizei)desc->data.subimage[face_index][mip_index].size;
              glCompressedTexImage3D(gl_img_target, mip_index,
                                     gl_internal_format, mip_width, mip_height,
                                     mip_depth, 0, data_size, data_ptr);
            } else {
              const GLenum gl_type =
                  _sg_gl_teximage_type(img->cmn.pixel_format);
...
_sg_gl_create_shader(_sg_shader_t *shd, const sg_shader_desc *desc) {
  ...
  for (int stage_index = 0; stage_index < SG_NUM_SHADER_STAGES; stage_index++) {
    ...
    for (int ub_index = 0; ub_index < stage->num_uniform_blocks; ub_index++) {
      ...
      for (int u_index = 0; u_index < SG_MAX_UB_MEMBERS; u_index++) {
        ...
        const uint32_t u_size = _sg_uniform_size(
            u_desc->type, u_desc->array_count, ub_desc->layout);
  ...
  for (int stage_index = 0; stage_index < SG_NUM_SHADER_STAGES; stage_index++) {
    ...
    const _sg_shader_stage_t *stage = &shd->cmn.stage[stage_index];
    _sg_gl_shader_stage_t *gl_stage = &shd->gl.stage[stage_index];
    for (int img_smp_index = 0; img_smp_index < stage->num_image_samplers;
         img_smp_index++) {
      const sg_shader_image_sampler_pair_desc *img_smp_desc =
          &stage_desc->image_sampler_pairs[img_smp_index];
      _sg_gl_shader_image_sampler_t *gl_img_smp =
          &gl_stage->image_samplers[img_smp_index];
      ...
      if (gl_loc != -1) {
        ...
        _SG_LOGMSG(GL_TEXTURE_NAME_NOT_FOUND_IN_SHADER,
...
_SOKOL_PRIVATE sg_resource_state _sg_gl_create_pipeline(
    _sg_pipeline_t *pip, _sg_shader_t *shd, const sg_pipeline_desc *desc) {
...
_SOKOL_PRIVATE void
_sg_gl_fb_attach_texture(const _sg_gl_attachment_t *gl_att,
...
_SOKOL_PRIVATE sg_resource_state
_sg_gl_create_attachments(_sg_attachments_t *atts, _sg_image_t **color_images,
                          _sg_image_t **resolve_images, _sg_image_t *ds_image,
                          const sg_attachments_desc *desc) {
  SOKOL_ASSERT(atts && desc);
  SOKOL_ASSERT(color_images && resolve_images);
  _SG_GL_CHECK_ERROR();
...
_SOKOL_PRIVATE void _sg_gl_apply_viewport(int x, int y, int w, int h,
                                          bool origin_top_left) {
  y = origin_top_left ? (_sg.cur_pass.height - (y + h)) : y;
  glViewport(x, y, w, h);
}

_SOKOL_PRIVATE void _sg_gl_apply_scissor_rect(int x, int y, int w, int h,
                                              bool origin_top_left) {
  y = origin_top_left ? (_sg.cur_pass.height - (y + h)) : y;
  glScissor(x, y, w, h);
}
...
_SOKOL_PRIVATE bool _sg_gl_apply_bindings(_sg_bindings_t *bnd) {
  ...
  for (int stage_index = 0; stage_index < SG_NUM_SHADER_STAGES; stage_index++) {
    ...
    const _sg_gl_shader_stage_t *gl_stage =
        &bnd->pip->shader->gl.stage[stage_index];
    _sg_image_t **imgs =
        (stage_index == SG_SHADERSTAGE_VS) ? bnd->vs_imgs : bnd->fs_imgs;
    _sg_sampler_t **smps =
        (stage_index == SG_SHADERSTAGE_VS) ? bnd->vs_smps : bnd->fs_smps;
    const int num_imgs = (stage_index == SG_SHADERSTAGE_VS) ? bnd->num_vs_imgs
                                                            : bnd->num_fs_imgs;
    const int num_smps = (stage_index == SG_SHADERSTAGE_VS) ? bnd->num_vs_smps
                                                            : bnd->num_fs_smps;
    ...
    for (int img_smp_index = 0; img_smp_index < stage->num_image_samplers;
         img_smp_index++) {
      const int gl_tex_slot =
          gl_stage->image_samplers[img_smp_index].gl_tex_slot;
      if (gl_tex_slot != -1) {
...
_SOKOL_PRIVATE void _sg_gl_apply_uniforms(sg_shader_stage stage_index,
                                          int ub_index, const sg_range *data) {
  SOKOL_ASSERT(_sg.gl.cache.cur_pipeline);
  SOKOL_ASSERT(_sg.gl.cache.cur_pipeline->slot.id ==
               _sg.gl.cache.cur_pipeline_id.id);
  SOKOL_ASSERT(_sg.gl.cache.cur_pipeline->shader->slot.id ==
               _sg.gl.cache.cur_pipeline->cmn.shader_id.id);
  SOKOL_ASSERT(_sg.gl.cache.cur_pipeline->shader->cmn.stage[stage_index]
                   .num_uniform_blocks > ub_index);
  SOKOL_ASSERT(_sg.gl.cache.cur_pipeline->shader->cmn.stage[stage_index]
                   .uniform_blocks[ub_index]
                   .size == data->size);
  const _sg_gl_shader_stage_t *gl_stage =
      &_sg.gl.cache.cur_pipeline->shader->gl.stage[stage_index];
...
_SOKOL_PRIVATE void _sg_gl_draw(int base_element, int num_elements,
                                int num_instances) {
  SOKOL_ASSERT(_sg.gl.cache.cur_pipeline);
...
_SOKOL_PRIVATE void _sg_gl_append_buffer(_sg_buffer_t *buf,
                                         const sg_range *data, bool new_frame) {
  SOKOL_ASSERT(buf && data && data->ptr && (data->size > 0));
  if (new_frame) {
    if (++buf->cmn.active_slot >= buf->cmn.num_slots) {
      buf->cmn.active_slot = 0;
...
static inline void
_sg_d3d11_OMSetRenderTargets(ID3D11DeviceContext *self, UINT NumViews,
...
static inline void
_sg_d3d11_RSSetState(ID3D11DeviceContext *self,
...
static inline void
_sg_d3d11_OMSetDepthStencilState(ID3D11DeviceContext *self,
...
static inline void
_sg_d3d11_IASetVertexBuffers(ID3D11DeviceContext *self, UINT StartSlot,
                             UINT NumBuffers,
...
static inline void
_sg_d3d11_VSSetConstantBuffers(ID3D11DeviceContext *self, UINT StartSlot,
                               UINT NumBuffers,
...
static inline void
_sg_d3d11_PSSetConstantBuffers(ID3D11DeviceContext *self, UINT StartSlot,
                               UINT NumBuffers,
                               ID3D11Buffer *const *ppConstantBuffers) {
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_VSSetSamplers(ID3D11DeviceContext *self, UINT StartSlot,
                        UINT NumSamplers,
...
static inline void
_sg_d3d11_PSSetSamplers(ID3D11DeviceContext *self, UINT StartSlot,
                        UINT NumSamplers,
                        ID3D11SamplerState *const *ppSamplers) {
#if defined(__cplusplus)
...
_sg_d3d11_CreateBuffer(ID3D11Device *self, const D3D11_BUFFER_DESC *pDesc,
...
static inline HRESULT
_sg_d3d11_CreateTexture2D(ID3D11Device *self, const D3D11_TEXTURE2D_DESC *pDesc,
                          const D3D11_SUBRESOURCE_DATA *pInitialData,
...
static inline HRESULT
_sg_d3d11_CreateShaderResourceView(ID3D11Device *self,
...
static inline HRESULT
_sg_d3d11_CreateTexture3D(ID3D11Device *self, const D3D11_TEXTURE3D_DESC *pDesc,
                          const D3D11_SUBRESOURCE_DATA *pInitialData,
...
static inline HRESULT
_sg_d3d11_CreateSamplerState(ID3D11Device *self,
...
static inline HRESULT
_sg_d3d11_CreateRasterizerState(ID3D11Device *self,
...
static inline HRESULT
_sg_d3d11_CreateBlendState(ID3D11Device *self,
...
static inline HRESULT
_sg_d3d11_CreateRenderTargetView(ID3D11Device *self, ID3D11Resource *pResource,
...
static inline HRESULT
_sg_d3d11_CreateDepthStencilView(ID3D11Device *self, ID3D11Resource *pResource,
...
static inline void
_sg_d3d11_ClearRenderTargetView(ID3D11DeviceContext *self,
...
static inline void
_sg_d3d11_ClearDepthStencilView(ID3D11DeviceContext *self,
                                ID3D11DepthStencilView *pDepthStencilView,
...
static inline void
_sg_d3d11_IASetPrimitiveTopology(ID3D11DeviceContext *self,
...
static inline void
_sg_d3d11_UpdateSubresource(ID3D11DeviceContext *self,
...
static inline void _sg_d3d11_Draw(ID3D11DeviceContext *self, UINT VertexCount,
...
_sg_d3d11_create_image(_sg_image_t *img, const sg_image_desc *desc) {
  ...
  if (img->cmn.type != SG_IMAGETYPE_3D) {
    ...
    if (injected) {
      ...
      // FIXME: currently we don't support setting MSAA texture as shader
      // resource
      if (!msaa) {
        D3D11_SHADER_RESOURCE_VIEW_DESC d3d11_srv_desc;
        _sg_clear(&d3d11_srv_desc, sizeof(d3d11_srv_desc));
        ...
        hr = _sg_d3d11_CreateShaderResourceView(
            _sg.d3d11.dev, (ID3D11Resource *)img->d3d11.tex2d, &d3d11_srv_desc,
            &img->d3d11.srv);
...
_sg_d3d11_compile_shader(const sg_shader_stage_desc *stage_desc) {
...
_sg_d3d11_create_shader(_sg_shader_t *shd, const sg_shader_desc *desc) {
  ...
  for (int stage_index = 0; stage_index < SG_NUM_SHADER_STAGES; stage_index++) {
    ...
    for (int ub_index = 0; ub_index < cmn_stage->num_uniform_blocks;
         ub_index++) {
      const _sg_shader_uniform_block_t *ub =
          &cmn_stage->uniform_blocks[ub_index];
      ...
      hr = _sg_d3d11_CreateBuffer(_sg.d3d11.dev, &cb_desc, NULL,
...
_SOKOL_PRIVATE sg_resource_state _sg_d3d11_create_pipeline(
    _sg_pipeline_t *pip, _sg_shader_t *shd, const sg_pipeline_desc *desc) {
  SOKOL_ASSERT(pip && shd && desc);
  SOKOL_ASSERT(desc->shader.id == shd->slot.id);
  ...
  for (int layout_index = 0; layout_index < SG_MAX_VERTEX_BUFFERS;
       layout_index++) {
...
_SOKOL_PRIVATE UINT _sg_d3d11_calcsubresource(UINT mip_slice, UINT array_slice,
                                              UINT mip_levels) {
...
_SOKOL_PRIVATE void _sg_d3d11_apply_viewport(int x, int y, int w, int h,
                                             bool origin_top_left) {
  SOKOL_ASSERT(_sg.d3d11.ctx);
  D3D11_VIEWPORT vp;
  vp.TopLeftX = (FLOAT)x;
...
_SOKOL_PRIVATE void _sg_d3d11_apply_scissor_rect(int x, int y, int w, int h,
                                                 bool origin_top_left) {
  SOKOL_ASSERT(_sg.d3d11.ctx);
  D3D11_RECT rect;
  rect.left = x;
...
_SOKOL_PRIVATE bool _sg_d3d11_apply_bindings(_sg_bindings_t *bnd) {
  ...
  for (int i = 0; i < bnd->num_vs_imgs; i++) {
    ...
    d3d11_vs_srvs[_SG_D3D11_SHADERSTAGE_IMAGE_SRV_OFFSET + i] =
        bnd->vs_imgs[i]->d3d11.srv;
  }
  for (int i = 0; i < bnd->num_vs_sbufs; i++) {
    SOKOL_ASSERT(bnd->vs_sbufs[i]->d3d11.srv);
    d3d11_vs_srvs[_SG_D3D11_SHADERSTAGE_BUFFER_SRV_OFFSET + i] =
        bnd->vs_sbufs[i]->d3d11.srv;
  }
  for (int i = 0; i < bnd->num_fs_imgs; i++) {
    SOKOL_ASSERT(bnd->fs_imgs[i]->d3d11.srv);
    d3d11_fs_srvs[_SG_D3D11_SHADERSTAGE_IMAGE_SRV_OFFSET + i] =
        bnd->fs_imgs[i]->d3d11.srv;
  }
  for (int i = 0; i < bnd->num_fs_sbufs; i++) {
    SOKOL_ASSERT(bnd->fs_sbufs[i]->d3d11.srv);
    d3d11_fs_srvs[_SG_D3D11_SHADERSTAGE_BUFFER_SRV_OFFSET + i] =
        bnd->fs_sbufs[i]->d3d11.srv;
  }
  ...
  _sg_d3d11_IASetVertexBuffers(_sg.d3d11.ctx, 0, SG_MAX_VERTEX_BUFFERS,
                               d3d11_vbs, bnd->pip->d3d11.vb_strides,
                               d3d11_vb_offsets);
  _sg_d3d11_IASetIndexBuffer(_sg.d3d11.ctx, d3d11_ib,
                             bnd->pip->d3d11.index_format,
                             (UINT)bnd->ib_offset);
  _sg_d3d11_VSSetShaderResources(_sg.d3d11.ctx, 0,
                                 _SG_D3D11_MAX_SHADERSTAGE_SRVS, d3d11_vs_srvs);
  _sg_d3d11_PSSetShaderResources(_sg.d3d11.ctx, 0,
                                 _SG_D3D11_MAX_SHADERSTAGE_SRVS, d3d11_fs_srvs);
  _sg_d3d11_VSSetSamplers(_sg.d3d11.ctx, 0, SG_MAX_SHADERSTAGE_SAMPLERS,
                          d3d11_vs_smps);
  _sg_d3d11_PSSetSamplers(_sg.d3d11.ctx, 0, SG_MAX_SHADERSTAGE_SAMPLERS,
                          d3d11_fs_smps);
...
_SOKOL_PRIVATE void _sg_d3d11_apply_uniforms(sg_shader_stage stage_index,
                                             int ub_index,
                                             const sg_range *data) {
  SOKOL_ASSERT(_sg.d3d11.ctx);
  ...
  SOKOL_ASSERT(_sg.d3d11.cur_pipeline->shader &&
               _sg.d3d11.cur_pipeline->shader->slot.id ==
                   _sg.d3d11.cur_pipeline->cmn.shader_id.id);
  SOKOL_ASSERT(ub_index < _sg.d3d11.cur_pipeline->shader->cmn.stage[stage_index]
                              .num_uniform_blocks);
  SOKOL_ASSERT(data->size ==
               _sg.d3d11.cur_pipeline->shader->cmn.stage[stage_index]
                   .uniform_blocks[ub_index]
                   .size);
  ID3D11Buffer *cb =
      _sg.d3d11.cur_pipeline->shader->d3d11.stage[stage_index].cbufs[ub_index];
  SOKOL_ASSERT(cb);
  _sg_d3d11_UpdateSubresource(_sg.d3d11.ctx, (ID3D11Resource *)cb, 0, NULL,
                              data->ptr, 0, 0);
  _sg_stats_add(d3d11.uniforms.num_update_subresource, 1);
}

_SOKOL_PRIVATE void _sg_d3d11_draw(int base_element, int num_elements,
                                   int num_instances) {
  const bool use_instanced_draw =
  ...
  for (int face_index = 0; face_index < num_faces; face_index++) {
    for (int slice_index = 0; slice_index < num_slices; slice_index++) {
      for (int mip_index = 0; mip_index < img->cmn.num_mipmaps;
        ...
        _sg_stats_add(d3d11.num_map, 1);
        if (SUCCEEDED(hr)) {
          // FIXME: need to handle difference in depth-pitch for 3D textures as
          // well!
...
_SOKOL_PRIVATE void
_sg_mtl_copy_image_data(const _sg_image_t *img,
...
_sg_mtl_create_shader(_sg_shader_t *shd, const sg_shader_desc *desc) {
  ...
  if (desc->vs.bytecode.ptr && desc->fs.bytecode.ptr) {
    ...
    vs_lib = _sg_mtl_library_from_bytecode(desc->vs.bytecode.ptr,
                                           desc->vs.bytecode.size);
    fs_lib = _sg_mtl_library_from_bytecode(desc->fs.bytecode.ptr,
...
_SOKOL_PRIVATE void _sg_mtl_discard_shader(_sg_shader_t *shd) {
  ...
  _sg_mtl_release_resource(_sg.frame_index,
                           shd->mtl.stage[SG_SHADERSTAGE_VS].mtl_func);
  _sg_mtl_release_resource(_sg.frame_index,
                           shd->mtl.stage[SG_SHADERSTAGE_VS].mtl_lib);
  _sg_mtl_release_resource(_sg.frame_index,
                           shd->mtl.stage[SG_SHADERSTAGE_FS].mtl_func);
  _sg_mtl_release_resource(_sg.frame_index,
                           shd->mtl.stage[SG_SHADERSTAGE_FS].mtl_lib);
}

_SOKOL_PRIVATE sg_resource_state _sg_mtl_create_pipeline(
    _sg_pipeline_t *pip, _sg_shader_t *shd, const sg_pipeline_desc *desc) {
  SOKOL_ASSERT(pip && shd && desc);
  SOKOL_ASSERT(desc->shader.id == shd->slot.id);
  ...
  for (NSUInteger attr_index = 0; attr_index < SG_MAX_VERTEX_ATTRIBUTES;
    ...
    vtx_desc.attributes[attr_index].bufferIndex =
        (NSUInteger)(a_state->buffer_index + SG_MAX_SHADERSTAGE_UBS);
    pip->cmn.vertex_buffer_layout_active[a_state->buffer_index] = true;
  }
  for (NSUInteger layout_index = 0; layout_index < SG_MAX_VERTEX_BUFFERS;
       layout_index++) {
    if (pip->cmn.vertex_buffer_layout_active[layout_index]) {
      const sg_vertex_buffer_layout_state *l_state =
          &desc->layout.buffers[layout_index];
  ...
  SOKOL_ASSERT(shd->mtl.stage[SG_SHADERSTAGE_VS].mtl_func !=
               _SG_MTL_INVALID_SLOT_INDEX);
  rp_desc.vertexFunction =
      _sg_mtl_id(shd->mtl.stage[SG_SHADERSTAGE_VS].mtl_func);
  SOKOL_ASSERT(shd->mtl.stage[SG_SHADERSTAGE_FS].mtl_func !=
               _SG_MTL_INVALID_SLOT_INDEX);
  rp_desc.fragmentFunction =
      _sg_mtl_id(shd->mtl.stage[SG_SHADERSTAGE_FS].mtl_func);
  ...
  if (@available(macOS 10.13, iOS 11.0, *)) {
    for (NSUInteger i = 0; i < (SG_MAX_SHADERSTAGE_UBS + SG_MAX_VERTEX_BUFFERS);
         i++) {
  ...
  id<MTLRenderPipelineState> mtl_rps =
      [_sg.mtl.device newRenderPipelineStateWithDescriptor:rp_desc error:&err];
...
_SOKOL_PRIVATE sg_resource_state
_sg_mtl_create_attachments(_sg_attachments_t *atts, _sg_image_t **color_images,
                           _sg_image_t **resolve_images, _sg_image_t *ds_img,
                           const sg_attachments_desc *desc) {
  SOKOL_ASSERT(atts && desc);
  SOKOL_ASSERT(color_images && resolve_images);
...
_SOKOL_PRIVATE void _sg_mtl_begin_pass(const sg_pass *pass) {
  ...
  /*
      ...
      NOTE: we're creating two command buffers here, one with unretained
     references for storing the regular commands, and one with retained
     references for storing the presentDrawable call (this needs to hold on the
     drawable until presentation has happened - and the easiest way to do this
     is to let the command buffer manage the lifetime of the drawable).
...
_SOKOL_PRIVATE void _sg_mtl_apply_viewport(int x, int y, int w, int h,
                                           bool origin_top_left) {
  SOKOL_ASSERT(nil != _sg.mtl.cmd_encoder);
...
_SOKOL_PRIVATE void _sg_mtl_apply_scissor_rect(int x, int y, int w, int h,
                                               bool origin_top_left) {
  SOKOL_ASSERT(nil != _sg.mtl.cmd_encoder);
...
_SOKOL_PRIVATE bool _sg_mtl_apply_bindings(_sg_bindings_t *bnd) {
  ...
  for (NSUInteger slot = 0; slot < (NSUInteger)bnd->num_vbs; slot++) {
    ...
    if ((_sg.mtl.state_cache.cur_vertexbuffer_ids[slot].id != vb->slot.id) ||
      ...
      if (_sg.mtl.state_cache.cur_vertexbuffer_ids[slot].id != vb->slot.id) {
        ...
        [_sg.mtl.cmd_encoder setVertexBufferOffset:(NSUInteger)vb_offset
                                           atIndex:mtl_slot];
      }
  ...
  for (NSUInteger slot = 0; slot < (NSUInteger)bnd->num_vs_sbufs; slot++) {
    ...
    if (_sg.mtl.state_cache.cur_vs_storagebuffer_ids[slot].id !=
        sbuf->slot.id) {
      _sg.mtl.state_cache.cur_vs_storagebuffer_ids[slot].id = sbuf->slot.id;
      SOKOL_ASSERT(sbuf->mtl.buf[sbuf->cmn.active_slot] !=
                   _SG_MTL_INVALID_SLOT_INDEX);
      const NSUInteger mtl_slot =
          SG_MAX_SHADERSTAGE_UBS + SG_MAX_VERTEX_BUFFERS + slot;
      [_sg.mtl.cmd_encoder
  ...
  for (NSUInteger slot = 0; slot < (NSUInteger)bnd->num_fs_sbufs; slot++) {
    ...
    if (_sg.mtl.state_cache.cur_fs_storagebuffer_ids[slot].id !=
        sbuf->slot.id) {
...
_SOKOL_PRIVATE void _sg_mtl_apply_uniforms(sg_shader_stage stage_index,
                                           int ub_index, const sg_range *data) {
  SOKOL_ASSERT(nil != _sg.mtl.cmd_encoder);
  ...
  SOKOL_ASSERT(_sg.mtl.state_cache.cur_pipeline &&
               _sg.mtl.state_cache.cur_pipeline->shader);
  ...
  SOKOL_ASSERT(_sg.mtl.state_cache.cur_pipeline->shader->slot.id ==
               _sg.mtl.state_cache.cur_pipeline->cmn.shader_id.id);
  SOKOL_ASSERT(ub_index <
               _sg.mtl.state_cache.cur_pipeline->shader->cmn.stage[stage_index]
                   .num_uniform_blocks);
  SOKOL_ASSERT(data->size ==
               _sg.mtl.state_cache.cur_pipeline->shader->cmn.stage[stage_index]
                   .uniform_blocks[ub_index]
  ...
  if (stage_index == SG_SHADERSTAGE_VS) {
    [_sg.mtl.cmd_encoder setVertexBufferOffset:(NSUInteger)_sg.mtl.cur_ub_offset
...
_SOKOL_PRIVATE void _sg_mtl_draw(int base_element, int num_elements,
                                 int num_instances) {
  SOKOL_ASSERT(nil != _sg.mtl.cmd_encoder);
  SOKOL_ASSERT(_sg.mtl.state_cache.cur_pipeline &&
...
_SOKOL_PRIVATE WGPUBufferUsageFlags _sg_wgpu_buffer_usage(sg_buffer_type t,
                                                          sg_usage u) {
...
_sg_wgpu_texture_sample_type(sg_image_sample_type t) {
  switch (t) {
...
//
_SOKOL_PRIVATE uint32_t _sg_wgpu_image_binding(sg_shader_stage stage,
                                               int img_slot) {
...
_SOKOL_PRIVATE uint32_t _sg_wgpu_sampler_binding(sg_shader_stage stage,
                                                 int smp_slot) {
...
_SOKOL_PRIVATE uint32_t _sg_wgpu_storagebuffer_binding(sg_shader_stage stage,
                                                       int sbuf_slot) {
...
_SOKOL_PRIVATE void _sg_wgpu_uniform_buffer_init(const sg_desc *desc) {
  ...
  WGPUBindGroupLayoutEntry ub_bgle_desc[SG_NUM_SHADER_STAGES]
                                       [SG_MAX_SHADERSTAGE_UBS];
  _sg_clear(ub_bgle_desc, sizeof(ub_bgle_desc));
  for (uint32_t stage_index = 0; stage_index < SG_NUM_SHADER_STAGES;
       stage_index++) {
    WGPUShaderStage vis = (stage_index == SG_SHADERSTAGE_VS)
    ...
    for (uint32_t ub_index = 0; ub_index < SG_MAX_SHADERSTAGE_UBS; ub_index++) {
      ...
      ub_bgle_desc[stage_index][ub_index].buffer.type =
  ...
  _sg.wgpu.uniform.bind.group_layout =
      wgpuDeviceCreateBindGroupLayout(_sg.wgpu.dev, &ub_bgl_desc);
  ...
  _sg.wgpu.uniform.bind.group =
      wgpuDeviceCreateBindGroup(_sg.wgpu.dev, &bg_desc);
...
_SOKOL_PRIVATE void _sg_wgpu_uniform_buffer_on_commit(void) {
  ...
  _sg_clear(&_sg.wgpu.uniform.bind.offsets[0][0],
            sizeof(_sg.wgpu.uniform.bind.offsets));
}
...
_SOKOL_PRIVATE void
_sg_wgpu_init_bindgroups_cache_key(_sg_wgpu_bindgroups_cache_key_t *key,
  ...
  const int fs_imgs_offset =
      vs_sbufs_offset + SG_MAX_SHADERSTAGE_STORAGEBUFFERS;
  ...
  SOKOL_ASSERT((fs_sbufs_offset + SG_MAX_SHADERSTAGE_STORAGEBUFFERS) ==
...
_SOKOL_PRIVATE bool
_sg_wgpu_compare_bindgroups_cache_key(_sg_wgpu_bindgroups_cache_key_t *k0,
...
_sg_wgpu_create_bindgroup(_sg_bindings_t *bnd) {
  ...
  int bge_index = 0;
  if (bnd->pip->is_compute) {
    for (int i = 0; i < bnd->num_cs_sbufs; i++) {
      WGPUBindGroupEntry *wgpu_entry = &wgpu_entries[bge_index++];
      wgpu_entry->binding =
          _sg_wgpu_storagebuffer_binding(SG_SHADERSTAGE_CS, i);
      wgpu_entry->buffer = bnd->cs_sbufs[i]->wgpu.buf;
      wgpu_entry->size = (uint64_t)bnd->cs_sbufs[i]->cmn.size;
    }
  } else {
    for (int i = 0; i < bnd->num_vs_imgs; i++) {
      WGPUBindGroupEntry *wgpu_entry = &wgpu_entries[bge_index++];
    ...
    for (int i = 0; i < bnd->num_vs_sbufs; i++) {
      WGPUBindGroupEntry *wgpu_entry = &wgpu_entries[bge_index++];
      wgpu_entry->binding =
          _sg_wgpu_storagebuffer_binding(SG_SHADERSTAGE_VS, i);
    ...
    for (int i = 0; i < bnd->num_fs_sbufs; i++) {
      WGPUBindGroupEntry *wgpu_entry = &wgpu_entries[bge_index++];
      wgpu_entry->binding =
          _sg_wgpu_storagebuffer_binding(SG_SHADERSTAGE_FS, i);
...
_SOKOL_PRIVATE bool _sg_wgpu_bindings_cache_vb_dirty(int index,
...
_SOKOL_PRIVATE void _sg_wgpu_bindings_cache_vb_update(int index,
                                                      const _sg_buffer_t *vb,
                                                      int offset) {
  SOKOL_ASSERT((index >= 0) && (index < SG_MAX_VERTEX_BUFFERS));
  if (vb) {
...
_SOKOL_PRIVATE bool _sg_wgpu_apply_bindgroup(_sg_bindings_t *bnd) {
  if ((bnd->num_vs_imgs + bnd->num_vs_smps + bnd->num_vs_sbufs +
       bnd->num_fs_imgs + bnd->num_fs_smps + bnd->num_fs_sbufs) > 0) {
    if (!_sg.desc.wgpu_disable_bindgroups_cache) {
      ...
      if (bg == 0) {
        // either no cache entry yet, or cache collision, create new bindgroup
        // and store in cache
...
_SOKOL_PRIVATE bool _sg_wgpu_apply_index_buffer(_sg_bindings_t *bnd) {
  if (_sg_wgpu_bindings_cache_ib_dirty(bnd->ib, bnd->ib_offset)) {
    ...
    if (bnd->ib) {
      ...
      wgpuRenderPassEncoderSetIndexBuffer(_sg.wgpu.pass_enc, bnd->ib->wgpu.buf,
                                          format, offset, max_bytes);
      _sg_stats_add(wgpu.bindings.num_set_index_buffer, 1);
    }
    // FIXME: else-path should actually set a null index buffer (this was just
    // recently implemented in WebGPU)
  } else {
...
_SOKOL_PRIVATE bool _sg_wgpu_apply_vertex_buffers(_sg_bindings_t *bnd) {
  for (int slot = 0; slot < bnd->num_vbs; slot++) {
    if (_sg_wgpu_bindings_cache_vb_dirty(slot, bnd->vbs[slot],
                                         bnd->vb_offsets[slot])) {
      _sg_wgpu_bindings_cache_vb_update(slot, bnd->vbs[slot],
                                        bnd->vb_offsets[slot]);
      const uint64_t buf_size = (uint64_t)bnd->vbs[slot]->cmn.size;
      const uint64_t offset = (uint64_t)bnd->vb_offsets[slot];
      SOKOL_ASSERT(buf_size > offset);
      const uint64_t max_bytes = buf_size - offset;
      wgpuRenderPassEncoderSetVertexBuffer(_sg.wgpu.pass_enc, (uint32_t)slot,
                                           bnd->vbs[slot]->wgpu.buf, offset,
                                           max_bytes);
  ...
  // FIXME: remaining vb slots should actually set a null vertex buffer (this
  // was just recently implemented in WebGPU)
  return true;
}
...
_sg_wgpu_create_shader(_sg_shader_t *shd, const sg_shader_desc *desc) {
  ...
  int bgl_index = 0;
  if (!desc->is_compute) {
    for (int stage_index = 0; stage_index < SG_NUM_SHADER_STAGES;
         stage_index++) {
      const sg_shader_stage_desc *stage_desc =
          (stage_index == SG_SHADERSTAGE_VS) ? &desc->vs : &desc->fs;
      ...
      wgpu_stage->module =
          wgpuDeviceCreateShaderModule(_sg.wgpu.dev, &wgpu_shdmod_desc);
      ...
      for (int img_index = 0; img_index < num_images; img_index++) {
        ...
        WGPUBindGroupLayoutEntry *wgpu_bgl_entry =
            &wgpu_bgl_entries[bgl_index++];
        const sg_shader_image_desc *img_desc = &stage_desc->images[img_index];
        wgpu_bgl_entry->binding =
            _sg_wgpu_image_binding((sg_shader_stage)stage_index, img_index);
        wgpu_bgl_entry->visibility =
            _sg_wgpu_shader_stage((sg_shader_stage)stage_index);
        wgpu_bgl_entry->texture.viewDimension =
            _sg_wgpu_texture_view_dimension(img_desc->image_type);
        wgpu_bgl_entry->texture.sampleType =
            _sg_wgpu_texture_sample_type(img_desc->sample_type);
      ...
      for (int smp_index = 0; smp_index < num_samplers; smp_index++) {
        ...
        WGPUBindGroupLayoutEntry *wgpu_bgl_entry =
            &wgpu_bgl_entries[bgl_index++];
        const sg_shader_sampler_desc *smp_desc =
            &stage_desc->samplers[smp_index];
        wgpu_bgl_entry->binding =
            _sg_wgpu_sampler_binding((sg_shader_stage)stage_index, smp_index);
        wgpu_bgl_entry->visibility =
            _sg_wgpu_shader_stage((sg_shader_stage)stage_index);
        wgpu_bgl_entry->sampler.type =
            _sg_wgpu_sampler_binding_type(smp_desc->sampler_type);
      }
      for (int sbuf_index = 0; sbuf_index < num_sbufs; sbuf_index++) {
        ...
        const sg_shader_storage_buffer_desc *sbuf_desc =
            &stage_desc->storage_buffers[sbuf_index];
        wgpu_bgl_entry->binding = _sg_wgpu_storagebuffer_binding(
            (sg_shader_stage)stage_index, sbuf_index);
        wgpu_bgl_entry->visibility =
            _sg_wgpu_shader_stage((sg_shader_stage)stage_index);
        wgpu_bgl_entry->buffer.type =
            sbuf_desc->readonly ? WGPUBufferBindingType_ReadOnlyStorage
  ...
  shd->wgpu.bind_group_layout =
      wgpuDeviceCreateBindGroupLayout(_sg.wgpu.dev, &wgpu_bgl_desc);
...
_SOKOL_PRIVATE sg_resource_state _sg_wgpu_create_pipeline(
    _sg_pipeline_t *pip, _sg_shader_t *shd, const sg_pipeline_desc *desc) {
  SOKOL_ASSERT(pip && shd && desc);
  SOKOL_ASSERT(desc->shader.id == shd->slot.id);
  ...
  wgpu_bgl[_SG_WGPU_UNIFORM_BINDGROUP_INDEX] =
      _sg.wgpu.uniform.bind.group_layout;
  wgpu_bgl[_SG_WGPU_IMAGE_SAMPLER_BINDGROUP_INDEX] =
      shd->wgpu.bind_group_layout;
  ...
  if (desc->is_compute) {
    ...
    WGPUComputePipelineDescriptor cp_desc = {
        ...
        .compute = {.module = shd->wgpu.stage[SG_SHADERSTAGE_CS].module,
                    .entryPoint = "main"},
    };
    printf("%s\n", shd->wgpu.stage[SG_SHADERSTAGE_CS].entry.buf);
    pip->wgpu.compute = wgpuDeviceCreateComputePipeline(_sg.wgpu.dev, &cp_desc);
    if (0 == pip->wgpu.compute) {
    ...
    WGPUVertexAttribute wgpu_vtx_attrs[SG_MAX_VERTEX_BUFFERS]
                                      [SG_MAX_VERTEX_ATTRIBUTES];
    _sg_clear(wgpu_vtx_attrs, sizeof(wgpu_vtx_attrs));
    int wgpu_vb_num = 0;
    for (int vb_idx = 0; vb_idx < SG_MAX_VERTEX_BUFFERS;
         vb_idx++, wgpu_vb_num++) {
    ...
    wgpu_pip_desc.vertex.entryPoint =
        shd->wgpu.stage[SG_SHADERSTAGE_VS].entry.buf;
  ...
  wgpuPipelineLayoutRelease(wgpu_pip_layout);
  if (0 == pip->wgpu.render) {
    _SG_ERROR(WGPU_CREATE_RENDER_PIPELINE_FAILED);
    return SG_RESOURCESTATE_FAILED;
  } else if (0 == pip->wgpu.compute) {
    _SG_ERROR(WGPU_CREATE_COMPUTE_PIPELINE_FAILED);
  }
  return SG_RESOURCESTATE_VALID;
}
...
//     wgpu_vtx_attrs[vb_idx][wgpu_attr_idx].format =
//         _sg_wgpu_vertexformat(va_state->format);
//     wgpu_vtx_attrs[vb_idx][wgpu_attr_idx].offset =
//     (uint64_t)va_state->offset;
//     wgpu_vtx_attrs[vb_idx][wgpu_attr_idx].shaderLocation = (uint32_t)va_idx;
//   }
//
//   WGPURenderPipelineDescriptor wgpu_pip_desc;
...
//   if (desc->color_count > 0) {
//     wgpu_frag_state.module = shd->wgpu.stage[SG_SHADERSTAGE_FS].module;
//     wgpu_frag_state.entryPoint =
//     shd->wgpu.stage[SG_SHADERSTAGE_FS].entry.buf; wgpu_frag_state.targetCount
//     = (size_t)desc->color_count; wgpu_frag_state.targets =
//     &wgpu_ctgt_state[0]; for (int i = 0; i < desc->color_count; i++) {
//       SOKOL_ASSERT(i < SG_MAX_COLOR_ATTACHMENTS);
//       wgpu_ctgt_state[i].format =
//           _sg_wgpu_textureformat(desc->colors[i].pixel_format);
//       wgpu_ctgt_state[i].writeMask =
...
//     wgpu_pip_desc.fragment = &wgpu_frag_state;
//   }
//   pip->wgpu.pip = wgpuDeviceCreateRenderPipeline(_sg.wgpu.dev,
//   &wgpu_pip_desc); wgpuPipelineLayoutRelease(wgpu_pip_layout); if (0 ==
//   pip->wgpu.pip) {
//     _SG_ERROR(WGPU_CREATE_RENDER_PIPELINE_FAILED);
//     return SG_RESOURCESTATE_FAILED;
//   }
...
_SOKOL_PRIVATE void _sg_wgpu_discard_pipeline(_sg_pipeline_t *pip) {
  ...
  if (pip->wgpu.render) {
    wgpuRenderPipelineRelease(pip->wgpu.render);
    pip->wgpu.render = 0;
  }
  if (pip->wgpu.compute) {
    wgpuComputePipelineRelease(pip->wgpu.compute);
    pip->wgpu.compute = 0;
  }
}

_SOKOL_PRIVATE sg_resource_state
_sg_wgpu_create_attachments(_sg_attachments_t *atts, _sg_image_t **color_images,
                            _sg_image_t **resolve_images, _sg_image_t *ds_img,
                            const sg_attachments_desc *desc) {
  SOKOL_ASSERT(atts && desc);
  SOKOL_ASSERT(color_images && resolve_images);
...
_SOKOL_PRIVATE void
_sg_wgpu_init_color_att(WGPURenderPassColorAttachment *wgpu_att,
...
_SOKOL_PRIVATE void
_sg_wgpu_init_ds_att(WGPURenderPassDepthStencilAttachment *wgpu_att,
                     const sg_pass_action *action, sg_pixel_format fmt,
                     WGPUTextureView view) {
  wgpu_att->view = view;
...
_SOKOL_PRIVATE void _sg_wgpu_begin_pass(const sg_pass *pass) {
  ...
  // initial uniform buffer binding (required even if no uniforms are set in the
  // frame)
...
SOKOL_API_IMPL void sg_wgpu_begin_compute_pass(void) {
  SOKOL_ASSERT(_sg.valid);
  SOKOL_ASSERT(_sg.backend == SG_BACKEND_WGPU);
  SOKOL_ASSERT(_sg.wgpu.cmd_enc);
  _sg.wgpu.comp_enc =
      wgpuCommandEncoderBeginComputePass(_sg.wgpu.cmd_enc, NULL);
  SOKOL_ASSERT(_sg.wgpu.comp_enc);
}

SOKOL_API_IMPL void sg_wgpu_end_compute_pass(void) {
  SOKOL_ASSERT(_sg.valid);
  SOKOL_ASSERT(_sg.backend == SG_BACKEND_WGPU);
  SOKOL_ASSERT(_sg.wgpu.cmd_enc);
  SOKOL_ASSERT(_sg.wgpu.comp_enc);
  wgpuComputePassEncoderEnd(_sg.wgpu.comp_enc);
  wgpuComputePassEncoderRelease(_sg.wgpu.comp_enc);
  _sg.wgpu.comp_enc = NULL;
}
...
_SOKOL_PRIVATE void _sg_wgpu_apply_viewport(int x, int y, int w, int h,
                                            bool origin_top_left) {
  SOKOL_ASSERT(_sg.wgpu.pass_enc);
  ...
  wgpuRenderPassEncoderSetViewport(_sg.wgpu.pass_enc, xf, yf, wf, hf, 0.0f,
                                   1.0f);
}

_SOKOL_PRIVATE void _sg_wgpu_apply_scissor_rect(int x, int y, int w, int h,
                                                bool origin_top_left) {
  SOKOL_ASSERT(_sg.wgpu.pass_enc);
  const _sg_recti_t clip =
      _sg_clipi(x, y, w, h, _sg.cur_pass.width, _sg.cur_pass.height);
...
_SOKOL_PRIVATE void _sg_wgpu_apply_pipeline(_sg_pipeline_t *pip) {
  SOKOL_ASSERT(pip);
  SOKOL_ASSERT(pip->wgpu.compute || pip->wgpu.render);
  SOKOL_ASSERT(_sg.wgpu.pass_enc);
  ...
  if (pip->is_compute) {
    SOKOL_ASSERT(pip->wgpu.compute);
    SOKOL_ASSERT(_sg.wgpu.comp_enc);
    wgpuComputePassEncoderSetPipeline(_sg.wgpu.comp_enc, pip->wgpu.compute);
  } else {
    SOKOL_ASSERT(pip->wgpu.render);
    SOKOL_ASSERT(_sg.wgpu.pass_enc);
    wgpuRenderPassEncoderSetPipeline(_sg.wgpu.pass_enc, pip->wgpu.render);
    wgpuRenderPassEncoderSetBlendConstant(_sg.wgpu.pass_enc,
                                          &pip->wgpu.blend_color);
    wgpuRenderPassEncoderSetStencilReference(_sg.wgpu.pass_enc,
                                             pip->cmn.stencil.ref);
  }
}
...
_SOKOL_PRIVATE void _sg_wgpu_apply_uniforms(sg_shader_stage stage_index,
                                            int ub_index,
                                            const sg_range *data) {
  ...
  SOKOL_ASSERT(_sg.wgpu.cur_pipeline->shader->slot.id ==
               _sg.wgpu.cur_pipeline->cmn.shader_id.id);
  SOKOL_ASSERT(
      ub_index <
      _sg.wgpu.cur_pipeline->shader->cmn.stage[stage_index].num_uniform_blocks);
  SOKOL_ASSERT(data->size <=
               _sg.wgpu.cur_pipeline->shader->cmn.stage[stage_index]
                   .uniform_blocks[ub_index]
                   .size);
  ...
  memcpy(_sg.wgpu.uniform.staging + _sg.wgpu.uniform.offset, data->ptr,
         data->size);
  _sg.wgpu.uniform.bind.offsets[stage_index][ub_index] =
      _sg.wgpu.uniform.offset;
  _sg.wgpu.uniform.offset = _sg_roundup_u32(
      _sg.wgpu.uniform.offset + (uint32_t)data->size, alignment);
  wgpuRenderPassEncoderSetBindGroup(
      _sg.wgpu.pass_enc, _SG_WGPU_UNIFORM_BINDGROUP_INDEX,
      _sg.wgpu.uniform.bind.group,
      SG_NUM_SHADER_STAGES * SG_MAX_SHADERSTAGE_UBS,
      &_sg.wgpu.uniform.bind.offsets[0][0]);
}

_SOKOL_PRIVATE void _sg_wgpu_draw(int base_element, int num_elements,
                                  int num_instances) {
  SOKOL_ASSERT(_sg.wgpu.pass_enc);
  ...
  if (SG_INDEXTYPE_NONE != _sg.wgpu.cur_pipeline->cmn.index_type) {
    wgpuRenderPassEncoderDrawIndexed(_sg.wgpu.pass_enc, (uint32_t)num_elements,
                                     (uint32_t)num_instances,
                                     (uint32_t)base_element, 0, 0);
  } else {
    wgpuRenderPassEncoderDraw(_sg.wgpu.pass_enc, (uint32_t)num_elements,
...
static inline sg_resource_state
_sg_create_pipeline(_sg_pipeline_t *pip, _sg_shader_t *shd,
                    const sg_pipeline_desc *desc) {
#if defined(_SOKOL_ANY_GL)
...
static inline sg_resource_state
_sg_create_attachments(_sg_attachments_t *atts, _sg_image_t **color_images,
                       _sg_image_t **resolve_images, _sg_image_t *ds_image,
                       const sg_attachments_desc *desc) {
#if defined(_SOKOL_ANY_GL)
...
static inline void _sg_apply_viewport(int x, int y, int w, int h,
                                      bool origin_top_left) {
#if defined(_SOKOL_ANY_GL)
...
static inline void _sg_apply_scissor_rect(int x, int y, int w, int h,
                                          bool origin_top_left) {
#if defined(_SOKOL_ANY_GL)
...
static inline void _sg_apply_uniforms(sg_shader_stage stage_index, int ub_index,
                                      const sg_range *data) {
#if defined(_SOKOL_ANY_GL)
...
static inline void _sg_draw(int base_element, int num_elements,
                            int num_instances) {
#if defined(_SOKOL_ANY_GL)
...
static inline void _sg_append_buffer(_sg_buffer_t *buf, const sg_range *data,
                                     bool new_frame) {
#if defined(_SOKOL_ANY_GL)
...
_SOKOL_PRIVATE bool _sg_validate_buffer_desc(const sg_buffer_desc *desc) {
  ...
  if (!injected && (desc->usage == SG_USAGE_IMMUTABLE)) {
    _SG_VALIDATE((0 != desc->data.ptr) && (desc->data.size > 0),
  ...
  if (desc->type == SG_BUFFERTYPE_STORAGEBUFFER) {
    _SG_VALIDATE(_sg.features.storage_buffer,
...
_SOKOL_PRIVATE void _sg_validate_image_data(const sg_image_data *data,
                                            sg_pixel_format fmt, int width,
                                            int height, int num_faces,
                                            int num_mips, int num_slices) {
#if !defined(SOKOL_DEBUG)
  _SOKOL_UNUSED(data);
  _SOKOL_UNUSED(fmt);
  _SOKOL_UNUSED(width);
...
_SOKOL_PRIVATE bool _sg_validate_sampler_desc(const sg_sampler_desc *desc) {
  ...
  _SG_VALIDATE(desc->min_filter != SG_FILTER_NONE,
               VALIDATE_SAMPLERDESC_MINFILTER_NONE);
  _SG_VALIDATE(desc->mag_filter != SG_FILTER_NONE,
...
_SOKOL_PRIVATE bool _sg_validate_shader_desc(const sg_shader_desc *desc) {
  ...
  _SG_VALIDATE(0 != desc->cs.source, VALIDATE_SHADERDESC_SOURCE);
#elif defined(SOKOL_METAL) || defined(SOKOL_D3D11)
  // on Metal or D3D11, must provide shader source code or byte code
  _SG_VALIDATE((0 != desc->vs.source) || (0 != desc->vs.bytecode.ptr),
               VALIDATE_SHADERDESC_SOURCE_OR_BYTECODE);
  _SG_VALIDATE((0 != desc->fs.source) || (0 != desc->fs.bytecode.ptr),
               VALIDATE_SHADERDESC_SOURCE_OR_BYTECODE);
#else
  ...
  for (int i = 0; i < SG_MAX_VERTEX_ATTRIBUTES; i++) {
    if (desc->attrs[i].name) {
      _SG_VALIDATE(strlen(desc->attrs[i].name) < _SG_STRING_SIZE,
                   VALIDATE_SHADERDESC_ATTR_STRING_TOO_LONG);
    }
    if (desc->attrs[i].sem_name) {
      _SG_VALIDATE(strlen(desc->attrs[i].sem_name) < _SG_STRING_SIZE,
                   VALIDATE_SHADERDESC_ATTR_STRING_TOO_LONG);
    }
  }
  // if shader byte code, the size must also be provided
  if (0 != desc->vs.bytecode.ptr) {
    _SG_VALIDATE(desc->vs.bytecode.size > 0,
                 VALIDATE_SHADERDESC_NO_BYTECODE_SIZE);
  }
  if (0 != desc->fs.bytecode.ptr) {
    _SG_VALIDATE(desc->fs.bytecode.size > 0,
                 VALIDATE_SHADERDESC_NO_BYTECODE_SIZE);
  }
  for (int stage_index = 0; stage_index < SG_NUM_SHADER_STAGES; stage_index++) {
    const sg_shader_stage_desc *stage_desc =
    ...
    for (int ub_index = 0; ub_index < SG_MAX_SHADERSTAGE_UBS; ub_index++) {
      const sg_shader_uniform_block_desc *ub_desc =
          &stage_desc->uniform_blocks[ub_index];
      if (ub_desc->size > 0) {
        _SG_VALIDATE(uniform_blocks_continuous,
        ...
        for (int u_index = 0; u_index < SG_MAX_UB_MEMBERS; u_index++) {
          ...
          if (u_desc->type != SG_UNIFORMTYPE_INVALID) {
            ...
            const uint32_t u_align = _sg_uniform_alignment(
                u_desc->type, array_count, ub_desc->layout);
    ...
    for (int sbuf_index = 0; sbuf_index < SG_MAX_SHADERSTAGE_STORAGEBUFFERS;
      ...
      const sg_shader_storage_buffer_desc *sbuf_desc =
          &stage_desc->storage_buffers[sbuf_index];
      if (sbuf_desc->used) {
        _SG_VALIDATE(storage_buffers_continuous,
                     VALIDATE_SHADERDESC_NO_CONT_STORAGEBUFFERS);
        _SG_VALIDATE(sbuf_desc->readonly,
    ...
    for (int img_smp_index = 0;
      ...
      const sg_shader_image_sampler_pair_desc *img_smp_desc =
          &stage_desc->image_sampler_pairs[img_smp_index];
      if (img_smp_desc->used) {
        _SG_VALIDATE(image_samplers_continuous,
                     VALIDATE_SHADERDESC_NO_CONT_IMAGE_SAMPLER_PAIRS);
        num_image_samplers++;
        const bool img_slot_in_range =
            (img_smp_desc->image_slot >= 0) &&
            (img_smp_desc->image_slot < SG_MAX_SHADERSTAGE_IMAGES);
        const bool smp_slot_in_range =
            (img_smp_desc->sampler_slot >= 0) &&
            (img_smp_desc->sampler_slot < SG_MAX_SHADERSTAGE_SAMPLERS);
        _SG_VALIDATE(
        ...
        if (img_slot_in_range && smp_slot_in_range) {
          const sg_shader_image_desc *img_desc =
              &stage_desc->images[img_smp_desc->image_slot];
          const sg_shader_sampler_desc *smp_desc =
              &stage_desc->samplers[img_smp_desc->sampler_slot];
          ...
          if (needs_comparison) {
            _SG_VALIDATE(needs_comparison && (smp_desc->sampler_type ==
                                              SG_SAMPLERTYPE_COMPARISON),
    ...
    for (int img_smp_index = 0; img_smp_index < num_image_samplers;
         img_smp_index++) {
      const sg_shader_image_sampler_pair_desc *img_smp_desc =
          &stage_desc->image_sampler_pairs[img_smp_index];
      actual_img_slot_mask |= (1 << ((uint32_t)img_smp_desc->image_slot & 31));
      actual_smp_slot_mask |=
          (1 << ((uint32_t)img_smp_desc->sampler_slot & 31));
    }
    _SG_VALIDATE(
  ...
  for (int att_index = 0; att_index < SG_MAX_COLOR_ATTACHMENTS; att_index++) {
    ...
    if (0 != img) {
      ...
      if (att_index == 0) {
        ...
        _SG_VALIDATE(color_width ==
                         _sg_miplevel_dim(img->cmn.width, att->mip_level),
                     VALIDATE_ATTACHMENTSDESC_IMAGE_SIZES);
        _SG_VALIDATE(color_height ==
...
_SOKOL_PRIVATE bool _sg_validate_apply_pipeline(sg_pipeline pip_id) {
  ...
  if (_sg.cur_pass.atts_id.id != SG_INVALID_ID) {
    ...
    for (int i = 0; i < pip->cmn.color_count; i++) {
      ...
      _SG_VALIDATE(pip->cmn.colors[i].pixel_format == att_img->cmn.pixel_format,
                   VALIDATE_APIP_COLOR_FORMAT);
    ...
    _SG_VALIDATE(pip->cmn.colors[0].pixel_format ==
                     _sg.cur_pass.swapchain.color_fmt,
                 VALIDATE_APIP_COLOR_FORMAT);
    _SG_VALIDATE(pip->cmn.depth.pixel_format ==
                     _sg.cur_pass.swapchain.depth_fmt,
                 VALIDATE_APIP_DEPTH_FORMAT);
...
_SOKOL_PRIVATE bool _sg_validate_apply_bindings(const sg_bindings *bindings) {
  ...
  for (int i = 0; i < SG_MAX_SHADERSTAGE_IMAGES; i++) {
    const _sg_shader_stage_t *stage =
        &pip->shader->cmn.stage[SG_SHADERSTAGE_VS];
    if (stage->images[i].image_type != _SG_IMAGETYPE_DEFAULT) {
      _SG_VALIDATE(bindings->vs.images[i].id != SG_INVALID_ID,
                   VALIDATE_ABND_VS_EXPECTED_IMAGE_BINDING);
      if (bindings->vs.images[i].id != SG_INVALID_ID) {
        const _sg_image_t *img =
            _sg_lookup_image(&_sg.pools, bindings->vs.images[i].id);
        _SG_VALIDATE(img != 0, VALIDATE_ABND_VS_IMG_EXISTS);
        if (img && img->slot.state == SG_RESOURCESTATE_VALID) {
          _SG_VALIDATE(img->cmn.type == stage->images[i].image_type,
  ...
  for (int i = 0; i < SG_MAX_SHADERSTAGE_SAMPLERS; i++) {
    ...
    if (stage->samplers[i].sampler_type != _SG_SAMPLERTYPE_DEFAULT) {
      _SG_VALIDATE(bindings->vs.samplers[i].id != SG_INVALID_ID,
                   VALIDATE_ABND_VS_EXPECTED_SAMPLER_BINDING);
      if (bindings->vs.samplers[i].id != SG_INVALID_ID) {
        const _sg_sampler_t *smp =
            _sg_lookup_sampler(&_sg.pools, bindings->vs.samplers[i].id);
  ...
  for (int i = 0; i < SG_MAX_SHADERSTAGE_STORAGEBUFFERS; i++) {
    ...
    if (stage->storage_buffers[i].used) {
      _SG_VALIDATE(bindings->vs.storage_buffers[i].id != SG_INVALID_ID,
                   VALIDATE_ABND_VS_EXPECTED_STORAGEBUFFER_BINDING);
      if (bindings->vs.storage_buffers[i].id != SG_INVALID_ID) {
        const _sg_buffer_t *sbuf =
            _sg_lookup_buffer(&_sg.pools, bindings->vs.storage_buffers[i].id);
  ...
  for (int i = 0; i < SG_MAX_SHADERSTAGE_IMAGES; i++) {
    const _sg_shader_stage_t *stage =
        &pip->shader->cmn.stage[SG_SHADERSTAGE_FS];
    if (stage->images[i].image_type != _SG_IMAGETYPE_DEFAULT) {
      _SG_VALIDATE(bindings->fs.images[i].id != SG_INVALID_ID,
                   VALIDATE_ABND_FS_EXPECTED_IMAGE_BINDING);
      if (bindings->fs.images[i].id != SG_INVALID_ID) {
        const _sg_image_t *img =
            _sg_lookup_image(&_sg.pools, bindings->fs.images[i].id);
  ...
  for (int i = 0; i < SG_MAX_SHADERSTAGE_SAMPLERS; i++) {
    const _sg_shader_stage_t *stage =
        &pip->shader->cmn.stage[SG_SHADERSTAGE_FS];
    if (stage->samplers[i].sampler_type != _SG_SAMPLERTYPE_DEFAULT) {
      _SG_VALIDATE(bindings->fs.samplers[i].id != SG_INVALID_ID,
                   VALIDATE_ABND_FS_EXPECTED_SAMPLER_BINDING);
      if (bindings->fs.samplers[i].id != SG_INVALID_ID) {
        const _sg_sampler_t *smp =
            _sg_lookup_sampler(&_sg.pools, bindings->fs.samplers[i].id);
  ...
  for (int i = 0; i < SG_MAX_SHADERSTAGE_STORAGEBUFFERS; i++) {
    const _sg_shader_stage_t *stage =
        &pip->shader->cmn.stage[SG_SHADERSTAGE_FS];
    if (stage->storage_buffers[i].used) {
      _SG_VALIDATE(bindings->fs.storage_buffers[i].id != SG_INVALID_ID,
                   VALIDATE_ABND_FS_EXPECTED_STORAGEBUFFER_BINDING);
      if (bindings->fs.storage_buffers[i].id != SG_INVALID_ID) {
        const _sg_buffer_t *sbuf =
            _sg_lookup_buffer(&_sg.pools, bindings->fs.storage_buffers[i].id);
...
_SOKOL_PRIVATE bool _sg_validate_apply_uniforms(sg_shader_stage stage_index,
                                                int ub_index,
                                                const sg_range *data) {
#if !defined(SOKOL_DEBUG)
  _SOKOL_UNUSED(stage_index);
  ...
  _SG_VALIDATE(ub_index < stage->num_uniform_blocks,
               VALIDATE_AUB_NO_UB_AT_SLOT);

  // check that the provided data size matches the uniform block size
  _SG_VALIDATE(data->size == stage->uniform_blocks[ub_index].size,
               VALIDATE_AUB_SIZE);
...
_sg_shader_desc_defaults(const sg_shader_desc *desc) {
  ...
  def.cs.entry = _sg_def(def.cs.entry, "_main");
#else
  def.vs.entry = _sg_def(def.vs.entry, "main");
  def.fs.entry = _sg_def(def.fs.entry, "main");
  def.cs.entry = _sg_def(def.cs.entry, "main");
#endif
#if defined(SOKOL_D3D11)
  if (def.vs.source) {
  ...
  if (!desc->is_compute) {
    for (int stage_index = 0; stage_index < SG_NUM_SHADER_STAGES;
      ...
      sg_shader_stage_desc *stage_desc =
          stage_index == SG_SHADERSTAGE_VS ? &def.vs : &def.fs;
      if (stage_desc) {
        for (int ub_index = 0; ub_index < SG_MAX_SHADERSTAGE_UBS; ub_index++) {
    ...
    for (int stage_index = 1; stage_index < 3; stage_index++) {
      sg_shader_stage_desc *stage_desc =
          stage_index == SG_SHADERSTAGE_CS ? &def.cs : &def.fs;
      if (stage_desc) {
        for (int ub_index = 0; ub_index < SG_MAX_SHADERSTAGE_UBS; ub_index++) {
          sg_shader_uniform_block_desc *ub_desc =
              &stage_desc->uniform_blocks[ub_index];
...
_SOKOL_PRIVATE sg_pass _sg_pass_defaults(const sg_pass *pass) {
  ...
  if (res.attachments.id == SG_INVALID_ID) {
    // this is a swapchain-pass
    res.swapchain.sample_count = _sg_def(
        res.swapchain.sample_count, _sg.desc.environment.defaults.sample_count);
    res.swapchain.color_format = _sg_def(
        res.swapchain.color_format, _sg.desc.environment.defaults.color_format);
    res.swapchain.depth_format = _sg_def(
        res.swapchain.depth_format, _sg.desc.environment.defaults.depth_format);
  }
...
SOKOL_API_IMPL int sg_query_row_pitch(sg_pixel_format fmt, int width,
                                      int row_align_bytes) {
  SOKOL_ASSERT(_sg.valid);
...
SOKOL_API_IMPL int sg_query_surface_pitch(sg_pixel_format fmt, int width,
                                          int height, int row_align_bytes) {
  SOKOL_ASSERT(_sg.valid);
  SOKOL_ASSERT((width > 0) && (height > 0));
  SOKOL_ASSERT((row_align_bytes > 0) && _sg_ispow2(row_align_bytes));
...
SOKOL_API_IMPL void sg_apply_viewport(int x, int y, int width, int height,
                                      bool origin_top_left) {
  SOKOL_ASSERT(_sg.valid);
  SOKOL_ASSERT(_sg.cur_pass.in_pass);
...
SOKOL_API_IMPL void sg_apply_viewportf(float x, float y, float width,
                                       float height, bool origin_top_left) {
  sg_apply_viewport((int)x, (int)y, (int)width, (int)height, origin_top_left);
}

SOKOL_API_IMPL void sg_apply_scissor_rect(int x, int y, int width, int height,
                                          bool origin_top_left) {
  SOKOL_ASSERT(_sg.valid);
  SOKOL_ASSERT(_sg.cur_pass.in_pass);
...
SOKOL_API_IMPL void sg_apply_scissor_rectf(float x, float y, float width,
                                           float height, bool origin_top_left) {
...
SOKOL_API_IMPL void sg_apply_bindings(const sg_bindings *bindings) {
  ...
  for (int i = 0; i < SG_MAX_SHADERSTAGE_IMAGES; i++, bnd.num_vs_imgs++) {
    if (bindings->vs.images[i].id) {
      ...
      if (bnd.vs_imgs[i]) {
        _sg.next_draw_valid &=
            (SG_RESOURCESTATE_VALID == bnd.vs_imgs[i]->slot.state);
      } else {
        _sg.next_draw_valid = false;
      }
    } else {
  ...
  for (int i = 0; i < SG_MAX_SHADERSTAGE_SAMPLERS; i++, bnd.num_vs_smps++) {
    if (bindings->vs.samplers[i].id) {
      ...
      if (bnd.vs_smps[i]) {
        _sg.next_draw_valid &=
            (SG_RESOURCESTATE_VALID == bnd.vs_smps[i]->slot.state);
      } else {
        _sg.next_draw_valid = false;
      }
    } else {
  ...
  for (int i = 0; i < SG_MAX_SHADERSTAGE_STORAGEBUFFERS;
       i++, bnd.num_vs_sbufs++) {
    if (bindings->vs.storage_buffers[i].id) {
      ...
      if (bnd.vs_sbufs[i]) {
        _sg.next_draw_valid &=
            (SG_RESOURCESTATE_VALID == bnd.vs_sbufs[i]->slot.state);
      } else {
        _sg.next_draw_valid = false;
      }
    } else {
  ...
  for (int i = 0; i < SG_MAX_SHADERSTAGE_IMAGES; i++, bnd.num_fs_imgs++) {
    if (bindings->fs.images[i].id) {
      ...
      if (bnd.fs_imgs[i]) {
        _sg.next_draw_valid &=
            (SG_RESOURCESTATE_VALID == bnd.fs_imgs[i]->slot.state);
      } else {
        _sg.next_draw_valid = false;
      }
    } else {
  ...
  for (int i = 0; i < SG_MAX_SHADERSTAGE_SAMPLERS; i++, bnd.num_fs_smps++) {
    if (bindings->fs.samplers[i].id) {
      ...
      if (bnd.fs_smps[i]) {
        _sg.next_draw_valid &=
            (SG_RESOURCESTATE_VALID == bnd.fs_smps[i]->slot.state);
      } else {
        _sg.next_draw_valid = false;
      }
    } else {
  ...
  for (int i = 0; i < SG_MAX_SHADERSTAGE_STORAGEBUFFERS;
    ...
    if (bindings->fs.storage_buffers[i].id) {
      ...
      if (bnd.fs_sbufs[i]) {
        _sg.next_draw_valid &=
            (SG_RESOURCESTATE_VALID == bnd.fs_sbufs[i]->slot.state);
      } else {
        _sg.next_draw_valid = false;
      }
    } else {
...
SOKOL_API_IMPL void sg_apply_uniforms(sg_shader_stage stage, int ub_index,
                                      const sg_range *data) {
  SOKOL_ASSERT(_sg.valid);
  SOKOL_ASSERT(_sg.cur_pass.in_pass);
...
SOKOL_API_IMPL void sg_draw(int base_element, int num_elements,
                            int num_instances) {
  SOKOL_ASSERT(_sg.valid);
  SOKOL_ASSERT(_sg.cur_pass.in_pass);
  ...
  if (shd) {
    for (int stage_idx = 0; stage_idx < SG_NUM_SHADER_STAGES; stage_idx++) {
      ...
      for (int ub_idx = 0; ub_idx < stage->num_uniform_blocks; ub_idx++) {
        sg_shader_uniform_block_desc *ub_desc =
            &stage_desc->uniform_blocks[ub_idx];
      ...
      for (int img_smp_idx = 0; img_smp_idx < stage->num_image_samplers;
           img_smp_idx++) {
        sg_shader_image_sampler_pair_desc *img_smp_desc =
            &stage_desc->image_sampler_pairs[img_smp_idx];
        const _sg_shader_image_sampler_t *img_smp =
            &stage->image_samplers[img_smp_idx];
...
sg_d3d11_query_shader_info(sg_shader shd_id) {
  ...
  if (shd) {
    for (int i = 0; i < SG_MAX_SHADERSTAGE_UBS; i++) {
      res.vs_cbufs[i] =
          (const void *)shd->d3d11.stage[SG_SHADERSTAGE_VS].cbufs[i];
      res.fs_cbufs[i] =
          (const void *)shd->d3d11.stage[SG_SHADERSTAGE_FS].cbufs[i];
    }
...
SOKOL_API_IMPL sg_wgpu_shader_info sg_wgpu_query_shader_info(sg_shader shd_id) {
  ...
  if (shd) {
    ...
    res.cs_mod = (const void *)shd->wgpu.stage[SG_SHADERSTAGE_CS].module;
...
sg_wgpu_query_pipeline_info(sg_pipeline pip_id) {
  ...
  const _sg_pipeline_t *pip = _sg_lookup_pipeline(&_sg.pools, pip_id.id);
  if (pip) {
    res.pip = (const void *)pip->wgpu.render;
  }
#else
  _SOKOL_UNUSED(pip_id);
#endif
...
SOKOL_API_IMPL void sg_wgpu_dispatch_compute_shader(uint32_t x, uint32_t y,
                                                    uint32_t z) {
  SOKOL_ASSERT(_sg.valid);
  SOKOL_ASSERT(_sg.backend == SG_BACKEND_WGPU);
  SOKOL_ASSERT(_sg.cur_pipeline.id != SG_INVALID_ID);
  ...
  wgpuComputePassEncoderDispatchWorkgroups(_sg.wgpu.comp_enc, x, y, z);
}
